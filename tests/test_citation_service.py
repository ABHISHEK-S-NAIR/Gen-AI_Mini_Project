...
def test_citation_detects_author_year_and_numeric() -> None:
    ...
    state.papers[paper_id] = IngestedPaper(paper_id=paper_id, filename="a.pdf", raw_text=text)
...
def test_citation_classifies_contrasting_sentence() -> None:
    ...
    state.papers[paper_id] = IngestedPaper(paper_id=paper_id, filename="b.pdf", raw_text=text)
...
def test_citation_returns_e007_when_none_found() -> None:
    paper_id = "p3"
    state.papers[paper_id] = IngestedPaper(paper_id=paper_id, filename="c.pdf", raw_text="No references here")
...
def test_citation_does_not_overtrigger_contrasting_on_however_alone() -> None:
    ...
    state.papers[paper_id] = IngestedPaper(paper_id=paper_id, filename="d.pdf", raw_text=text)
...
def test_citation_context_cleans_metadata_noise() -> None:
    ...
    state.papers[paper_id] = IngestedPaper(paper_id=paper_id, filename="e.pdf", raw_text=text)
